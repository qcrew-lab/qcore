...
class ConstantWaveform(Waveform):
    ...
    def __init__(self, name: str, amp: float):
        self.amp = amp
        super().__init__(name=name, func="constant_fn", func_params={"amp": amp})

    def _create_yaml_map(self):
        yaml_map = {"name": self.name, "amp": self.amp}
        return yaml_map
...
DEFAULT_GAUSS_WF = ArbitraryWaveform(
    name="gauss_wf", func="gauss_fn", max_amp=0.25, sigma=1000, multiple_of_sigma=4
)

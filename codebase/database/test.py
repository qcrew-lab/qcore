...
initialise_today_database_at(name=database_name, 
                             path=database_path)
...
v = ParameterWithSetpoints('v',
                        get_cmd=None,
                        setpoints=(f, a),
                        vals=vals.Arrays(shape=(n_points,)))
i_avg = ParameterWithSetpoints('i_avg',
                        get_cmd=None,
                        setpoints=(f,),
                        vals=vals.Arrays(shape=(n_points,)))
...
with meas.run() as datasaver:
    ...
    for i in np.linspace(0,10, 11):
        datasaver.add_result((avg_n, i),(i_avg, raw_vals),(f, f_vals)) # raw_vals is the in each avg_n, the returned array

...
from codebase.instruments import QuantumElement
from codebase.utils.pulselib import (
    Pulse,
    MeasurementPulse,
    Waveform,
    ConstantWaveform,
...
def _build_element_ports_config(element: QuantumElement, element_schema: dict):
    ...
    if "in" in element.ports:  # single input
        element_schema["singleInput"]["port"] = (CONTROLLER, element.ports["in"])
        ...
        del (element_schema["singleInput"], element_schema["singleInputCollection"])
...
def _build_readout_element_config(element: QuantumElement, element_schema: dict):
    tof = (
        element.time_of_flight
        if element.time_of_flight is not None
        else DEFAULT_TIME_OF_FLIGHT
    )
    element_schema["time_of_flight"] = tof

    smearing = element.smearing if element.smearing is not None else DEFAULT_SMEARING
    ...
    for element in elements:
        for pulse in element.operations.values():
            if pulse.name not in pulses_config:  # only add unique pulses
                pulse_schema = deepcopy(QM_CONFIG_SCHEMA["pulses"]["pulse_name"])

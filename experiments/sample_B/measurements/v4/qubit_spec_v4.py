...
reload(cfg)
reload(stg)  # reloads modules before executing the code below

##########################        DATA SAVING VARIABLES       ##########################

SAMPLE_NAME = "sample_b"
EXP_NAME = "qubit_spec"
PROJECT_FOLDER_NAME = "smaple_b_test"
...
metadata = {
    ...
    "wait": 80000,  # delay between reps in ns, an integer multiple of 4 >= 16
    "start": -80e6,  # frequency sweep range is set by start, stop, and step
    "stop": -30e6,
    "step": 0.03e6,
...
with program() as qubit_spec:
    ...
    with for_(n, 0, n < metadata["reps"], n + 1):
        with for_(x, x_start, x < x_stop + x_step / 2, x + x_step):
            play(metadata["qubit_op"] * metadata["qubit_ampx"], stg.qubit.name)
